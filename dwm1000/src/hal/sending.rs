use crate::{DWM1000, Error, Ready, Sending, time::Instant};
    ...
    SPI: SpiDevice,
{
    ...
    /// macro, or you can use it in tandem with [`DWM1000::enable_tx_interrupts`]
    ...
    pub fn finish_sending(mut self) -> Result<DWM1000<SPI, Ready>, (Self, Error<SPI>)> {
        ...
        match self.spi.ec_ctrl().modify(|_, w| w.ostsm(0)) {
            Ok(_) => {}
