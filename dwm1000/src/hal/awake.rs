    ...
    State: Awake,
{
    ...
    pub(super) fn force_idle(&mut self, double_buffered: bool) -> Result<(), Error<SPI, CS>> {
        ...
        if double_buffered {
            ...
            self.spi.sys_mask().modify(|r, w| {
                saved_sys_mask = r.0.clone();
                w.mrxfce(0).mrxfcg(0).mrxdfr(0).mldedone(0)
            })?;
        }

        self.spi.sys_ctrl().write(|w| w.trxoff(0b1))?;
        while self.spi.sys_ctrl().read()?.trxoff() == 0b1 {}
    ...
    pub(crate) fn read_otp(&mut self, address: u16) -> Result<u32, Error<SPI, CS>> {
        ...
        self.spi.otp_ctrl().write(|w| w.otprden(0b1).otpread(0b1))?;
        self.spi.otp_ctrl().write(|w| w.otprden(0b1))?;
        // Read back value
        let value = self.spi.otp_rdat().read()?.value();
        // End read mode
        self.spi.otp_ctrl().write(|w| w)?;
        Ok(value)
    }
}
