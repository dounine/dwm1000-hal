    ...
    CS: OutputPin,
{
    ...
    pub fn set_sync_behaviour(&mut self, behaviour: SyncBehaviour) -> Result<(), Error<SPI, CS>> {
        match behaviour {
            SyncBehaviour::None => {
                // Disable all
                self.spi.ec_ctrl().modify(|_, w| w.osrsm(0).ostrm(0))?;
                // Disable the rx pll
                self.spi.pmsc_ctrl1().modify(|_, w| w.pllsyn(0))?;
            }
            SyncBehaviour::TimeBaseReset => {
                // Enable the rx pll
                self.spi.pmsc_ctrl1().modify(|_, w| w.pllsyn(1))?;
    ...
    pub fn send(
        ...
        self.spi.evc_ctrl().write(|w| w.evc_en(0b1))?;
        while self.spi.evc_ctrl().read()?.evc_en() == 0b1 {}
        ...
        match send_time {
            SendTime::Delayed(time) => {
                ...
                self.spi.dx_time().write(|w| w.value(time.value()))?;
            }
            SendTime::OnSync => {
                self.spi.ec_ctrl().modify(|_, w| w.wait(33).ostsm(1))?;
            }
            _ => {}
        }
    ...
    pub fn enter_sleep(
        ...
        if let Some(sd) = sleep_duration {
            ...
            // Set the counter
            self.spi.aon_cfg0().write(|w| w.sleep_tim(sd))?;
            // Enable the sleep counter
            self.spi.aon_cfg1().write(|w| w.sleep_cen(1).lposc_cal(1))?;
            // Upload array
            self.spi.aon_ctrl().write(|w| w.upl_cfg(1))?;
            self.spi.aon_ctrl().write(|w| w.upl_cfg(0))?;
        ...
        self.spi.aon_ctrl().write(|w| w)?;
        self.spi.aon_ctrl().write(|w| w.save(1))?;
