...
use dwm1000::spi::Error;
use nrf52832_hal::spi::Spi;
#[cfg(feature = "dev")]
use nrf52832_hal::{
    gpio::{
        p0::{P0_05, P0_11},
...
pub fn new_dw1000<SCK, MOSI, MISO, CS>(
...
) -> DWM1000<MySpim, p0::P0_17<Output<PushPull>>, dwm1000::Uninitialized> {
    ...
    let cs = cs.into_push_pull_output(Level::High);
    let spi = MySpim(spim,cs);

    DWM1000::new(spi)
}
///hello
pub struct MySpim(Spim<SPIM2>,P0_17<Output<PushPull>>);
///hello
pub enum MySpimError {
}

impl Debug for MySpimError {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        f.write_str("spim error")
    }
}

impl embedded_hal::spi::Error for MySpimError {
    fn kind(&self) -> ErrorKind {
        ErrorKind::Other
    }
}
impl ErrorType for MySpim {
    type Error = MySpimError;
}

impl SpiDevice for MySpim {
    fn transaction(&mut self, operations: &mut [Operation<'_, u8>]) -> Result<(), Self::Error> {
        for op in operations {
            match op {
                Operation::Read(d) => {
                    self.0.read(d).unwrap();
                }
                Operation::Write(d) => {
                    self.0.write(d).unwrap();
                }
                Operation::Transfer(read_buf, write_buf) => {
                    self.0.transfer(read_buf, write_buf)?;
                }
                Operation::TransferInPlace(d) => {
                    self.0.transfer_in_place(d).unwrap();
                }
...
pub struct DWM1001 {
    ...
    pub DW1000: DWM1000<MySpim, p0::P0_17<Output<PushPull>>, dwm1000::Uninitialized>,
